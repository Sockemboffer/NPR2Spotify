...
# editionStartYear = 1996
# editionDayData = list()
# editionYearLinkCache = NPRPageParser.LoadJSONFile("MoWeEd Article Link Cache/" + str(editionStartYear) + " MoWeEd Article Link Cache.json")
...
while startDate.month != 11:
    ...
    if editionDay == None:
    ...
    elif playlistURI in editionDay:
        # updating track info
        print("- - we would update")
    elif playlistURI not in editionDay and searchKey in editionDay:
        ...
        print(">> Interlude Tracks for {0} ".format(startDate.date()))
        print("\n")
        startDate = startDate + timedelta(days=+1)
